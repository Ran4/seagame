(Note: the code might have moved since the bug report was given).
  ...
  game.ts:204-206 — When dispatching commands from menu clicks:
  const actorId = menuItem.targetActorId !== undefined
    ? world.selectedActorId      // interaction → command goes to selected actor
    : (world.contextMenu.actorId ?? world.selectedActorId);
  This is correct in intent, but if world.selectedActorId is null at this point (e.g., the actor was deselected between opening the menu and clicking), the command targets nobody and silently fails since member will be null.

  ---
  
  The most impactful ones to fix are #2 (double mood roll), #6 (camera X unclamped), #7 (water scroll direction), and #8 (parrot pathfinding). Want me to fix any of these?
